...
export function DataTable({ rows, columns, empty = "No records found" }: { rows: AnyRow[]; columns: ColumnConfig[]; empty?: string }) {
  return (
    <div className="overflow-hidden rounded-lg border border-white/80 bg-white/95 shadow-premium backdrop-blur">
        ...
        <table className="min-w-full divide-y divide-slate-200/80 text-sm">
          ...
          <tbody className="divide-y divide-slate-100">
            {rows.length === 0 ? (
              ...
              rows.map((row, index) => (
                <tr key={row.id ?? index} className="bg-white transition hover:bg-orange-50/40">
                  {columns.map((column) => (
